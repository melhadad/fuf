...
;;; ------------------------------------------------------------
;;; File        : GR7.L
;;; Description : Grammar with types/fset/external/bk-class
;;; Author      : Michael Elhadad
;;; Created     : 21 Feb 91 (Derived from gr6)
;;; Modified    : 25 Feb 91 - Multiple bk-class allowed
...
(defun gsetup7 ()
  ...
  (setq *u-grammar*'((alt
				      ...
				      (alt (trace relative) (:index mood) 
					   ...
					   (((mood simple-relative)
					    ...
					    ((mood embedded-relative)
					     ...
					     (prep ((cat prep)))
					     (scope ((gap yes)))
					     (relative-marker ((cat question-pronoun)
							       (semantics {^ ^ scope semantics})
							       (syntax {^ ^ scope syntax})))
					     (alt scope-embedded 
						  (:index (scope role))
			...
			;; ao competes with manner for the manner-comp adverbial position
			(alt ao-adverbial (:demo "Can AO be conveyed by an adverbial")
			     (bk-class ao)
			     (((ao none))
			      ((ao ((ao-conveyed any))))
			      ((manner-comp ((cat adv)
