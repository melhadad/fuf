...
;;;               THIS IS NOT PART OF FUF.  IT IS NOT UNDER COPYRIGHT.
...
(defvar boxgray "0")      ;; 0 is black, 1 is white
(defvar boxkind "stroke")  ;; stroke for outline, fill for solid
(defvar edgewidth "1")    ;; .5 is thin lines, 1 is fairly thick
(defvar edgegray "0")     ;; dark, but not solid black, lines
...
(defun psgraph (root childf infof &optional (shrink nil) (insert nil))
  ...
  (cond (shrink
	  ...
	  (format t "pagewidth dup maximum-x max div ~
                     ...
                     min dup scale~%"
		  (- maximum-y minimum-y))
;	  (let ((reduction (min (/ (max pagewidth maximum-x)
;				   pagewidth)
;			        (/ (max pageheight (- maximum-y minimum-y))
;				   pageheight))))
;            (format t "~D ~D scale~%" reduction reduction))
...
(defun walk-graph (root)
  (when (eql num-psnodes max-psnodes)
	(error "More than ~D nodes in graph.  Graphing aborted." max-psnodes))
