...
(defun PUNCTUATE (sentence question-flag)
  ...
  ;; of the sentence
  (let ((lastword (car (last sentence))))
    (unless (and (punctuation-word lastword)
		 (not (equal (last-char lastword) #\,)))
  ...
  (let ((original-cat cat)
    ...
    (list-if-atom
     (case cat
       ...
       (pronoun
	(let ((pronoun-type (incr-gdp 'pronoun-type fd path)))
	  (morph-pronoun
	   lex
	   (if pronoun-type pronoun-type original-cat)
...
(defun morphology-help (&optional unknown-cats)
  ...
  (format
   ...
   "The categories known by the morphology module are: ~@
    ...
    adv, adj:     inflected on comparative and superlative.~@
...
(defun CHECK-DIGIT (digit)
  "Return digit, DEFAULT is YES"
  (cond ((member digit '(yes no roman)) digit)
	(t 'yes)))

;;; YD -
;;; reflexive
(defun check-reflexive-type (reflexive)
  "Returns reflexivity ty[e - REFLEXIVE or RECIPROCAL, reflexive is DEFAULT"
  (cond ((member reflexive '(reflexive reciprocal)) reflexive)
        (t 'reflexive)))

;;; superlative, compatative
(defun check-superlative (superlative)
  "Retrun superlative, DEFAULT is NO"
  (cond ((member superlative '(yes)) 'superlative)
        (t 'no)))

(defun check-comparative (comperative)
  "Retrun compatrative, DEFAULT is NO"
  (cond ((member comperative '(yes)) 'comparative)
        (t 'no)))
...
;;=============
;; Morph-procedures for binding- reflexives. (YD)

(defun morph-binding (fd binding path)
  "Returns the correct pronoun - including reflexives, given the features."
  (let* ((forced (incr-gdp 'forced-pronoun fd path))
         (lex (unless forced (incr-gdp 'lex fd path)))
         (pronoun-type (if forced 'personal (incr-gdp 'pronoun-type fd path)))
         (reflexive (incr-gdp 'reflexive-type fd path)))
  (cond ((and (eq binding 'ppro) (or (eq reflexive 'none) (null reflexive)))

         ;;; Here in morph-pronoun, we should also check if it was forced to be
         ;;; a pronoun, if yes, lex should be ignored.
         (morph-pronoun
          lex
          pronoun-type
          (check-case (incr-gdp 'case fd path))
        ...
        (t
         (morph-anaphor
          ...
          (check-animate (incr-gdp 'animate fd path))
          (check-person (incr-gdp 'person fd path))
          (check-reflexive-type reflexive))))))
...
(defun morph-anaphor (lex case gender number animate person reflexive-type)
  ...
  (cond
	 ...
	 (setq lex (cond ((lexfetch lex 'reflexive)) (t lex)))
	 (list lex))

        ((eq reflexive-type 'reciprocal)
         ;;; constraints here should be: 1. (eq person 'plural)
...
(defun MORPH-PRONOUN (lex pronoun-type case gender number
  ...
  (cond ((and (stringp lex)
	...
	((or (eq pronoun-type 'personal) (eq pronoun-type 'personal-pronoun))
	 ...
	 lex)
	((or (eq pronoun-type 'demonstrative) (eq pronoun-type 'demonstrative-pronoun))
	 (cond ((subsume 'plural number)
		(if (eq distance 'far) "those" "these"))
	       ((eq distance 'far) "that")
	       (t "this")))
	((or (eq pronoun-type 'relative) (eq pronoun-type 'relative-pronoun))
	 (when (eq animate 'no) (setq gender 'neuter))
	 (cond ((and (eq restrictive 'yes)
                     (eq animate 'no)) "that")
		...
		(if (eq gender 'neuter) "which" "whom"))
	       ...
	       ((eq animate 'no) "which")
	       (t "who")))
	((or (eq pronoun-type 'question) (eq pronoun-type 'question-pronoun))
	 (when (eq animate 'no) (setq gender 'neuter))
	 (cond ((eq restrictive 'yes) "which")
		...
		(if (eq gender 'neuter) "what" "whom"))
	       ...
	       ((eq animate 'no) "what")
	       (t "who")))
	((or (eq pronoun-type 'quantified) (eq pronoun-type 'quantified-pronoun))
	 lex)))
